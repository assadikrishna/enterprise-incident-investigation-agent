...
def make_reasoning_node(generate: GenerateFunction):
    ...
    def reasoning_node(
        ...
        prompt = (
            ...
            "- Review the previous actions and observations before "
            "selecting the next action.\n"
            "- Do not repeat a search that has already returned the "
            "same evidence unless there is a specific new reason.\n"
            "- If incident details and direct log evidence are available, "
            "consider whether you can finish with a useful report.\n"
            "- A final report may contain hypotheses and uncertainty; "
            "do not claim that an unverified root cause is proven.\n"
            "- A deployment occurring before an incident establishes "
            "timing, not causation. Do not attribute the root cause "
            "to the deployment without supporting evidence.\n"
            "- Distinguish the observed failure mechanism from its "
            "underlying cause. If the underlying cause is unknown, "
            "state that explicitly.\n"
            "- If further verification requires unavailable data or "
            "tools, describe it as a recommended human investigation "
            "step rather than repeatedly searching the same data.\n\n"
        )
